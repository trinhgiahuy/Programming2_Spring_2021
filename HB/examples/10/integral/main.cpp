...
// Command decltype gives the type of any expression (e.g. variable or function).
...
// «left» and «right» by using so called rectangular integration (or Midpoint Rule).
// Parameter «number_of_partitions» tells into how many equal-width parts the
// interval to be integrated will be partitioned, i.e. how many rectangles constitute
// the total integral (when calculated as the sum of rectangles).
...
int main() {
    // Note that the first parameters in the calls below are not the return values of
    // sin, cos, sqrt, and polynomial, but these functions themselves.
