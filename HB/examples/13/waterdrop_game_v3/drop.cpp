...
void Drop::addWater(std::vector<Splash>& createdSplashes)
{
    ...
    if( waterAmount_ > MAX_CAPACITY ) {
        ...
        // Drop removes the pointer to itself from the board. Then the reference counter
        // of the dynamically created object becomes zero and the object is destructed
        // automatically. Therefore this call must be the last command of the method.
...
{
    // CHARACTERS array contains symbols for each amount of water in the corresponding
    // indices, and thus, we can find the desired symbol easily.
