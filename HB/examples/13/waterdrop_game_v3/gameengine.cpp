...
void GameEngine::initialize()
{
    ...
    // Initializing the data content of the game board in the same way as in waterdrop_game_v2
...
void GameEngine::addWater(int x, int y)
{
    ...
    if (drop == nullptr) {
        ...
        // In the case of an empty square, add a new drop image to the drawing area
    ...
    // Updating the graphical view of the game board, moving splashes and adding water
    // from splashes to drops when necessary
    do {
        ...
        // Test printing to console: Put to use by removing comment in the next line
        ...
        for (auto const& drop : growingDrops) {
            ...
            if (dropAt(drop->getX(), drop->getY()) == nullptr) {
                // If added water made a drop to pop, the image of the drop will be removed
...
void GameEngine::moveSplashes(std::vector<std::shared_ptr<Drop> >& grownDrops)
{
    ...
    // They cannot be inserted to container splashes_ immediately, because they must
    // wait until the next iteration.
    std::vector<Splash> newSplashes;

    // All the splashes on the board will be gone through by the iterator in the while loop.
    ...
    while (r != splashes_.end()) {
        ...
        // Calling move for each splash, which returns true, if the splash moves
        //  outside the game board and becomes removable. This return value is saved.
        bool remove = r->move();
        ...
        // If a splash instance is removed from the vector, erase returns new iterator
        // that points the vector at the point, where traversal will be continued after
        // removal. Otherwise iterator takes a step forward.
        if(remove) {
            r = splashes_.erase(r);
        } else {
            r++;
...
char GameEngine::droplessSquareChar(int x, int y, std::vector<Splash>& splashes) const
{
    // Checking if the square has splashes. Saving the printing character of each
    // splash to string characters.
    ...
    if(characters.length() == 0) { // No splashes in the square -> printing emptyä
        return ' ';
    } else if (characters.length() == 1){  // A single splash -> printing its character
