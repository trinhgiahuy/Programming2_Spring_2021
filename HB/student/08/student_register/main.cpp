...
void change_phone(std::map< std::string, Student* >& student_numbers, std::map< std::string, Student* >& user_ids, std::string student) {
	std::string new_phone_number = "";	
	if (student_numbers.find(student) == student_numbers.end())	{
	...
	if (is_valid_phone_number(new_phone_number)){
		student_numbers.at(student)->phone_number = new_phone_number;
		user_ids.at(student_numbers.at(student)->user_id)->phone_number = new_phone_number;
	}
	
}

void print_to_file(std::string file_name, const std::map< std::string, Student* >& user_ids) {
	std::ofstream file_object(file_name, std::ios::out);
	for (auto student_id : user_ids) {
		file_object << student_id.second->student_number << ";"
			<< student_id.second->user_id << ";"
			<< student_id.second->name << ";"
			<< student_id.second->phone_number << ";"
			<< student_id.second->email << ";"
			<< student_id.second->skype << std::endl;
	}
	file_object.close();
}
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            if(parts.size() != 2){
				...
				change_phone(student_numbers, user_ids, parts.at(1));
				print_to_file(file_name, user_ids);
			}
