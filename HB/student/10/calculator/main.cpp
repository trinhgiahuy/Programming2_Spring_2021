...
bool Execute_Command(vector<string>& pieces, Command s) {
	if (pieces.size() - 1 == s.parameter_number) {
		...
		if(string_to_double(pieces.at(1), parameter_1) and string_to_double(pieces.at(2), parameter_2)){
			cout << s.action(parameter_1, parameter_2) << endl;
		}
		else {
...
int main() {
    ...
    while ( true ) {
		...
		for (auto s : COMMANDS) {
			if (stringToUpper(command_to_be_executed) == s.str) {
				command_found = s;
				known_command = true;
				break;
			}
		...
		if (known_command) {
			if (!Execute_Command(pieces, command_found)) {
				return EXIT_SUCCESS;
			}
		}
