...
/* Check all in put is number and (not) 0
 *
 * Parameters:
 *  s   			string to check
 *  zero_allowed    allow number to be zero flag
 *
 * Returns:
 *  True if string is a number, false is not
 */
bool Cli::is_numeric(std::string s, bool zero_allowed)
{
	// all-digits-are-zeros flag
	bool all_zero = true;
	...
	if (not zero_allowed and all_zero) {
		return false;
	}
    return true;
}
